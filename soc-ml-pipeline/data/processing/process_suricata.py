...
TABLE_NAME = "wazuh_events"  # <-- write to a real table (not 'alerts' view)
...
with open(LOCAL_RAW_FILE, "r", encoding="utf-8", errors="ignore") as f:
    for line in f:
        line = line.strip()
        if not line:
            continue
...
with open(LOCAL_DEDUP_FILE, "w", encoding="utf-8") as out_f:
    for alert in deduped_alerts:
        json.dump(alert, out_f, ensure_ascii=False)
        out_f.write("\n")
...
if not os.path.exists(CONFIG_FILE):
    sys.exit(f"[x] {CONFIG_FILE} not found. Please provide it with 'db_columns' and 'json_paths'.")
...
def insert_alert(cursor, alert):
    try:
        ...
        cursor.execute(f"""
            INSERT OR IGNORE INTO {TABLE_NAME} ({cols}) VALUES ({placeholders})
        """, values)
        return True
...
print(f"[✔] Successfully inserted {inserted} unique alerts into {DB_FILE}.{TABLE_NAME}")
